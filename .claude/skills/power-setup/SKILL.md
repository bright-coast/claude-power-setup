...
Ask which mail provider(s) they use via AskUserQuestion, presenting all four as real, visible options in the same question: Gmail, Outlook, Apple Mail, or other/IMAP. This has been observed going wrong live: a session presented Google Workspace as the only option and the user had to interrupt to name their real provider. Do not ask a leading yes/no like "are you on Google Workspace?", do not silently assume Gmail because it's the common case, and do not shorten the menu even if earlier answers (email address domain, company name, etc.) hint at a likely provider. Skip entirely if `personaTier` is `minimal` and they said "personal" with no mention of needing email access.
...
- **Outlook**: for mail, covered by the same `@marlinjai/email-mcp` wizard as Gmail, no meaningfully different setup: it uses a built-in OAuth/PKCE flow, so no Azure app registration is needed for the default path. If they proceed, run the same command as Gmail, `npx @marlinjai/email-mcp setup`, and pick Outlook when its wizard asks. Only mention Azure app registration as an optional aside, for someone who specifically wants their own OAuth app instead of the built-in one.
  - If they also want Outlook **calendar or contacts**, not just mail: `@marlinjai/email-mcp` is mail-only and doesn't cover that. The realistic path researched for this is `@littlebearapps/outlook-assistant` (mail + calendar + contacts together), but unlike the mail-only wizard above, this one genuinely does require the user to do their own Azure App Registration at portal.azure.com (add Microsoft Graph delegated permissions, generate a client secret), there's no pre-registered app for it. *NEEDS LIVE TEST: researched from the package's own docs, not run end-to-end through this interview yet; walk through it once yourself before leaning on it with a client.* If they proceed, check `claude mcp list` first, look up the real current install command and setup steps the same way as Apple Mail below (`npm view @littlebearapps/outlook-assistant` or its README, don't guess), then walk them through the Azure App Registration screens as the README actually describes them before adding it with `claude mcp add`.
- **Apple Mail / iCloud Mail**: ask first whether Claude Code is actually running on a Mac with Mail.app already signed into the account, or on Windows/Linux, or on a Mac but without that account configured in Mail.app, because the right package differs:
  - **On a Mac, with the account already set up in Mail.app**: recommend `apple-mail-mcp` (by sweetrb), local AppleScript, no credentials needed at all beyond what's already in Mail.app/Keychain. There's no single confirmed one-line install for this one, so don't guess a command. First run `claude mcp list` to check whether it's already configured. If it isn't, look up the package's actual install instructions before doing anything, for example `npm view apple-mail-mcp` for the current version and repo link, or its README at the linked GitHub repo, then add it with `claude mcp add` using whatever invocation that lookup actually confirms.
  - **Not on a Mac, or no local Mail.app account, or they also want Calendar/Contacts, not just Mail**: iCloud Mail, Calendar, and Contacts all support a much simpler standard path than a custom Apple Developer app: an app-specific password (generated at appleid.apple.com, under Sign-In and Security, requires 2FA already being on) used over plain IMAP for mail and CalDAV/CardDAV for calendar/contacts. Apple has no OAuth API and no developer-registration screen for third-party access to this data at all, so there's nothing to register. Researched option for this: `icloud-mcp` (by MrGo2, published to npm as `mcp-icloud`), which runs in a cross-platform "cloud mode" over IMAP/CalDAV/CardDAV and covers mail + calendar + contacts together (its local/AppleScript mode is Mac-only and out of scope here). *NEEDS LIVE TEST: researched from the package's own docs, not run end-to-end through this interview yet; walk through it once yourself before leaning on it with a client.* Same approach as `apple-mail-mcp`: check `claude mcp list` first, look up the real current install command and setup steps (`npm view mcp-icloud` or its README, don't guess), walk the user through generating the app-specific password at appleid.apple.com if they don't already have one, then add it with `claude mcp add`.
  - If a bespoke Apple Developer "app" plus a custom OAuth-style flow has already been built for iCloud access for someone: that's very likely more than iCloud actually requires or even supports, since Apple has no public OAuth API for Mail/Calendar/Contacts data, only the app-specific-password route above. **Confirm with Rob** whether there was a specific reason the simpler app-specific-password path didn't fit (for example needing Notes, Reminders, Messages, or Safari data, which only the local AppleScript route can reach, not cloud IMAP/CalDAV/CardDAV) before assuming one approach simply replaces the other.
- **Other/IMAP**: note they'll need an app-specific password from their provider's security settings (not their real account password), and point to `imap-mcp-server` (by nikolausm), which also has a built-in iCloud preset if this is where an iCloud user without local Mail.app access ends up (mail only; for iCloud calendar/contacts too, use the Apple Mail branch above instead). Same approach as Apple Mail: check `claude mcp list` first, then look up the real install command with `npm view imap-mcp-server` or its README rather than guessing, then add it with `claude mcp add`.
